...
class MotionGhost extends Component {
  ...
  handleMouseMove (event) {
    ...
    const sorted = [...this.state.ghosts, 1.0, 0.0].sort((a, b) =>
      Math.abs(a - tx) - Math.abs(b - tx))
        ...
        <svg
          ...
          <g transform={`translate(${0.5 * width - 0.5 * timelineWidth},100)`}>
            ...
            {this.state.ghosts.map((t, i) =>
              <Entity key={makeKey(t, i)} t={t} width={timelineWidth}
          ...
          <g transform={`translate(${0.5 * width - 0.5 * timelineWidth}, 300)`}>
            <line
              x1={-timelineMargins} y1='0'
              x2={timelineWidth + timelineMargins} y2='0'
              strokeWidth={5}
              style={{
              ...
              {interpolatedStyles =>
                <g>
                  {interpolatedStyles.map(config => {
                    ...
                    return (
                      <circle cx={timelineWidth * t} cy='0' r={x * 7.0} key={config.key}
                        style={{
                          fill: (t === currentTime
                            ? options.pointSelectedColor : options.pointColor)
                        }} />
                    )
                  })}
                </g>
            ...
            <circle cx={0} cy='0' r='12'
              style={{fill: options.edgeColor, strokeWidth: 2, stroke: '#fff'}} />
            <circle cx={timelineWidth} cy='0' r='12'
              style={{fill: options.edgeColor, strokeWidth: 2, stroke: '#fff'}} />
